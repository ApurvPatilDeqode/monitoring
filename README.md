# Monitoring over NGINX server

# **Solution**
We have NGINX server on one ubuntu ec2 instances that we want to monitor. So, we have installed `prometheus-nginx-exporter` and `node-exporter` on those instances, so that it can scrape NGINX and H/w and metrics to prometheus. 
Now, We need an ec2 instance to install our monitoring tools (prometheus server and Grafana) and created custom configuration file of prometheus which monitors the NGINX service running on other ubuntu ec2 instances. We have also configured Grafana server with Node_Exporter dashboard which gives elaborative visualization of NGINX server. 
With alertmanager we have also achieved to receive email notifications on service failure. 
...
- VPC 
- Subnet
- Security Group (Ports: SSH-22 , HTTPS-443 , CustomTCP-8080,9100,3000,9093,9090,4040)
...
- Setting up proxy-server in-order to communicate with prometheus server.
...
- Setup prometheus-nginx-exporter
   ...
   **NOTE:** Please refer `prometheus-nginx-logexporter.hcl` for adjustments. 
   **NOTE:** Please refer `prometheus-nginxlog-exporter.service` for `-config-file` path settings.
...
- Install node-exporter over NGINX's instance
   ...
   * The Node Exporter service should always start when the server boots so it will always be available to be scrapped for information. Create the node_exporter.service file with nano:
   ...
   ```
   * Please refer **node_exporter.service** file.
...
- Install prometheus server as follow
   ...
   * The only part remaining is to make Prometheus a systemd service so that we can easily manage its running status.Using your favorite text editor, create a systemd service file:
   ...
   ```
   * Please refer the **prometheus.service** file.
...
- Install alertmanager server & service:
   * Download the version of Alertmanager with the following command:
   ```
   ...
   * Now, you have to create a systemd service file for Alert Manager so that you can easily manage (start, stop, restart, and add to startup) the alertmanager service with systemd.To create a systemd service file alertmanager.service, run the following command:
   ...
   ```
   * Please refer **alertmanager.service** file.
...
## Custom Configurations:
   ...
   - **4.Grafana**Once you log into grafana:
